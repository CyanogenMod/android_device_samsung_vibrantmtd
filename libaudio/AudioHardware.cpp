...
void AudioHardware::loadRILD(void)
{
    ...
    if (mSecRilLibHandle) {
        ...
        if (!openClientRILD  || !disconnectRILD   || !closeClientRILD ||
            !isConnectedRILD || !connectRILD      || !invokeOemRequestHookRaw) {
    ...
    if (status == NO_ERROR) {
        ...
        if (mMode == AudioSystem::MODE_IN_CALL)
        {
            if ((!mActivatedCP) && (mSecRilLibHandle) && (connectRILDIfRequired() == OK)) {
                ...
                data[2] = 0x00;
                data[3] = 0x05;
                data[4] = SOUND_CLOCK_START;
                invokeOemRequestHookRaw(mRilClient, data, sizeof(data));
                mActivatedCP = true;
            }
        }
    ...
    if ( (AudioSystem::MODE_IN_CALL == mMode) && (mSecRilLibHandle) &&
        ...
        switch (device) {
            case AudioSystem::DEVICE_OUT_EARPIECE:
                ...
                type = OEM_SND_TYPE_VOICE;
                break;

            case AudioSystem::DEVICE_OUT_SPEAKER:
                ALOGD("### speaker call volume");
                type = OEM_SND_TYPE_SPEAKER;
                break;
        ...
        if (mMode == AudioSystem::MODE_IN_CALL) {
            ...
            switch(device){
                case AudioSystem::DEVICE_OUT_EARPIECE:
                    ...
                    path = OEM_SND_AUDIO_PATH_HANDSET;
                    break;

                case AudioSystem::DEVICE_OUT_SPEAKER:
                    ALOGD("### incall mode speaker route");
                    path = OEM_SND_AUDIO_PATH_SPEAKER;
                    break;
                ...
                case AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_CARKIT:
                    ...
                    if (mBluetoothNrec) {
                        path = OEM_SND_AUDIO_PATH_BLUETOOTH;
                    } else {
                        path = OEM_SND_AUDIO_PATH_BT_NSEC_OFF;
                    }
                    break;

                case AudioSystem::DEVICE_OUT_WIRED_HEADPHONE :
                    ALOGD("### incall mode headphone route");
                    path = OEM_SND_AUDIO_PATH_HEADPHONE;
                    break;
                case AudioSystem::DEVICE_OUT_WIRED_HEADSET :
                    ALOGD("### incall mode headset route");
                    path = OEM_SND_AUDIO_PATH_HEADSET;
                    break;
                default:
